...
class BankView(discord.ui.View):
    ...
    async def loan_button(self, button, interaction):
        ...
        await interaction.response.edit_message(
            content=f"💳 대출 메뉴\n남은 대출 가능 금액: {self.remaining_limit:,}원\n얼마를 대출할까요?",
...
async def open_bank_menu(interaction: discord.Interaction):
    ...
    embed.add_field(
        name="💳 총 대출 한도",
        value=f"{info['loan_limit']:,}원 (레벨 {info['level']})",
        inline=False
    )
    embed.add_field(
        name="💰 현재 사용",
        value=f"{info['total_loans']:,}원",
        inline=False
    )
    ...
    if info["loans"]:
        ...
        for loan in info["loans"]:
            ...
            desc += (
                f"[#{loan['loan_id']}] "
                f"대출금: {loan['amount']:,}원 / 남은 상환금: {loan['remaining']:,}원 "
    ...
    embed.set_footer(text=f"잔액 : {info['balance']:,}원")
...
class LoanAmountModal(discord.ui.Modal):
    def __init__(self, user_id: int, remaining_limit: int):
        ...
        self.amount_input = discord.ui.InputText(
            label=f"얼마를 대출할까요? (남은 한도: {remaining_limit:,}원)",
...
async def show_loan_terms(interaction: discord.Interaction, user_id: int):
    embed = discord.Embed(
        title="📜 플그 카지노 대출 설명서",
        description=(
            "1️⃣ 대출 한도는 **레벨 × 10,000원**입니다.\n"
            "2️⃣ 대출 시 **이자율 10%**, 14일 내 상환 필요\n"
            "3️⃣ **대출 중 상점 이용 불가**\n"
            "4️⃣ **14일 초과 시 연체**, 추가 이자 +10% 부과\n"
            "5️⃣ 장기 연체 상태에서는 매일 00시 강제 상환됩니다.\n"
            "6️⃣ 장기 연체 중에도 **게임/돈줘는 가능**, 그러나 강제 상환 후 10,000원만 남습니다.\n\n"
...
class RepaySelectView(discord.ui.View):
    def __init__(self, user_id: int, loans: list):
        ...
        for loan in loans[:25]:
            label = f"#{loan['loan_id']} | {loan['amount']:,}원 남은 금액:{loan['remaining']:,}원"
